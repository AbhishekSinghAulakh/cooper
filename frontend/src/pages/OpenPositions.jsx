...
const OpenPositions = () => {
  ...
  const loadPortfolioHistory = async () => {
      try {
          ...
          // ⚡️ FIX: No need for explicit weekend filtering here if backend prevents weekend snapshots ⚡️
          // The backend's `ORDER BY date DESC LIMIT 1` will naturally pick Friday's data on Monday.
          // The `portfolioHistory` will only contain weekday data if snapshots are only taken on weekdays.
          setPortfolioHistory(historyData);

          if (historyData.length > 0) {
              const sortedHistory = [...historyData].sort((a, b) => new Date(a.date) - new Date(b.date));
              setBaseSnapshot(sortedHistory[0]);
  ...
  const handleTakeSnapshot = async () => {
      try {
          ...
          // ⚡️ IMPROVEMENT: Display backend error message more clearly ⚡️
          const errorMessage = err.response && err.response.data && err.response.data.detail
                               ? err.response.data.detail
                               : "Error taking snapshot. Check console for details.";
          alert(errorMessage);
      }
  };
