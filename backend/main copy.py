...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute("DROP TABLE IF EXISTS positions") # Consider uncommenting and deleting DB for schema change
        c.execute("""
...
def load_raw_excel_data():
    ...
    try:
        ...
        # ⚡️ IMPORTANT CHANGE HERE: Robust currency column parsing ⚡️
        # List all columns that might contain currency symbols or commas
        currency_like_cols = [
            'current_price', 'avg_price', 'delta', 'daily_change', 'daily_pnl',
            'tradevalue', 'market_value', 'total_pnl', # Add any other such columns
        ]
        for col in currency_like_cols:
            if col in df.columns:
                # Convert to string, remove '₹', ',', and any other non-numeric characters (except dot and minus)
                # This regex targets non-digit, non-dot, non-minus characters
                df[col] = df[col].astype(str).str.replace(r'[₹,]', '', regex=True)
                # Attempt conversion to numeric, coerce errors to NaN, then fill NaN with 0.0
                df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0.0)
            else:
                df[col] = 0.0 # Add column with 0.0 if not present

        # Other numeric columns (which should already be clean numbers or handled by string_cols)
        other_numeric_cols = [
        ...
        # Refined string column handling
        string_cols = ['sector', 'account', 'ticker', 'ms_ticker'] # Added 'ms_ticker' as it was in your excel sample
...
async def get_open_positions():
    ...
    result_list = []
    # Removed sorting by daily_change from backend, frontend will handle it
...
# Removed helper function for sorting as frontend will handle it
